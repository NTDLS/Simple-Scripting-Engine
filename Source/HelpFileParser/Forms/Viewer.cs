...
namespace HtmlHelpViewer
{
	...
	public class Viewer : System.Windows.Forms.Form, IHelpViewer
	{
		...
		public HtmlHelpSystem Reader = null;
        ...
        {
            this.helpSearch2.Search(searchText);
		...
		public Viewer()
		{
			...
			// create a new instance of the classExamples's main class
			Reader = new HtmlHelpSystem();
		...
		private void InitializeComponent()
		{
            ...
            this.tabControl1.Size = new System.Drawing.Size(300, 470);
            ...
            this.tabContents.Size = new System.Drawing.Size(292, 444);
            ...
            this.tocTree1.Size = new System.Drawing.Size(292, 444);
            ...
            this.tabIndex.Size = new System.Drawing.Size(292, 444);
            ...
            this.helpIndex1.Size = new System.Drawing.Size(292, 444);
            ...
            this.tabSearch.Size = new System.Drawing.Size(292, 444);
            ...
            this.helpSearch2.Size = new System.Drawing.Size(292, 444);
            ...
            this.panel3.Size = new System.Drawing.Size(451, 470);
            ...
            this.axWebBrowser1.Size = new System.Drawing.Size(451, 470);
            ...
            this.miCloseFile.Visible = false;
		...
		{
			RegistryKey regKey = Registry.LocalMachine.CreateSubKey(global.RegistryKeyString + "\\IDE\\Help");
		...
		private void helpSearch2_FTSearch(object sender, SearchEventArgs e)
		{
			...
			try
			{
				DataTable dtResults = Reader.PerformSearch( e.Words, 500, e.PartialWords, e.TitlesOnly);
        ...
        public IndexTopic GetTopic(string keyWord)
        {
			IndexItem foundIdx = Reader.Index.SearchIndex(keyWord + "?", IndexType.KeywordLinks);
            if (foundIdx != null)
            {
		...
		void IHelpViewer.ShowHelp(string namespaceFilter, HelpNavigator hlpNavigator, string keyword, string url)
		{
			switch(hlpNavigator)
			{
				...
				{
					IndexItem foundIdx = Reader.Index.SearchIndex(keyword, IndexType.AssiciativeLinks);
					if(foundIdx != null)
					{
				...
				case HelpNavigator.Find:
				{
					...
					DataTable dtResults = Reader.PerformSearch(keyword, 500, true, false);
				...
				{
					TOCItem foundTOC = Reader.TableOfContents.SearchTopic(keyword);
					if(foundTOC != null)
					{
        ...
        public string GetRegistryString(string sName, string Default)
        {
            ...
            RegistryKey regSubKey = regKey.OpenSubKey(global.RegistryKeyString + "\\IDE\\Help");
        ...
        */
        ...
        public bool OpenFile(string fileName)
        {
            ...
            try
            {
                ...
                Reader.OpenFile(fileName, _dmpInfo);

                // Enable the toc-tree pane if the opened file has a table of contents
                tocTree1.Enabled = Reader.HasTableOfContents;
                // Enable the index pane if the opened file has an index
                helpIndex1.Enabled = Reader.HasIndex;
                // Enable the full-text search pane if the opened file supports full-text searching
                helpSearch2.Enabled = Reader.FullTextSearch;

                btnContents.Enabled = Reader.HasTableOfContents;
                btnIndex.Enabled = Reader.HasIndex;
                btnSearch.Enabled = Reader.FullTextSearch;

                miContents.Enabled = Reader.HasTableOfContents;
                miContents1.Enabled = Reader.HasTableOfContents;
                miIndex.Enabled = Reader.HasIndex;
                miIndex1.Enabled = Reader.HasIndex;
                miSearch.Enabled = Reader.FullTextSearch;
                miSearch1.Enabled = Reader.FullTextSearch;
                btnSynch.Enabled = Reader.HasTableOfContents;

                tabControl1.SelectedIndex = 0;

                btnRefresh.Enabled = true;
                if (Reader.DefaultTopic.Length > 0)
                {
                    btnHome.Enabled = true;
                    miHome.Enabled = true;
                }
                ...
                tocTree1.BuildTOC(Reader.TableOfContents, _filter);

                // Build the index entries in the classlibrary control
                if (Reader.HasKLinks)
                    helpIndex1.BuildIndex(Reader.Index, IndexType.KeywordLinks, _filter);
                else if (Reader.HasALinks)
                    helpIndex1.BuildIndex(Reader.Index, IndexType.AssiciativeLinks, _filter);

                // Navigate the embedded browser to the default help topic
                NavigateBrowser(Reader.DefaultTopic);

                miMerge.Enabled = true;
                miCloseFile.Enabled = true;

                this.Text = Reader.FileList[0].FileInfo.HelpWindowTitle;

                miCustomize.Enabled = (Reader.HasInformationTypes || Reader.HasCategories);
		...
		private void miMerge_Click(object sender, System.EventArgs e)
		{
			if (openFileDialog1.ShowDialog() == DialogResult.OK )
			{
				...
				try
				{
					...
					Reader.MergeFile( openFileDialog1.FileName, _dmpInfo );
