...
namespace HtmlHelpViewer
{
	...
	public class Viewer : System.Windows.Forms.Form, IHelpViewer
	{
		...
		private string LM_Key = "SOFTWARE\\NetWorkDLS\\Simple Scripting Engine";
		...
		private void Form1_Load(object sender, System.EventArgs e)
		{
            ...
            try
            {
                ...
                string helpPath = GetRegistryString("Path", null);
                if (helpPath != null)
                {
                    string sDocumentation = helpPath + "\\Help\\Help.chm";

                    OpenFile(sDocumentation);

                    if (args.GetUpperBound(0) > 0)
                    {
                        this.helpSearch2.Search(args[1]);
            ...
            catch
            {
                while (true)
                {
                    MessageBox.Show("Failed to open the specified help file, please select the proper file.", "HTML Help Viewer");
